"""
Generated DAG: pipeline__eng_youtube_transfer

================================================================================
👶 어린이를 위한 100% 이해 가능 설명서 (Kindergarten Guide) 👶
================================================================================

안녕! 이 파일은 "DAG(댁)"이라고 불러요.
로봇 친구들에게 "어떤 일을, 어떤 순서로 해야 해!"라고 알려주는 *보물 지도* 같은 거예요.

우리가 하려는 일은 두 가지예요:
1. 유튜브에서 새로운 영상을 찾아오기 (fetch_videos)
2. 가장 최신 영상의 **전체 대본(Full Script)**을 가져오기 (process_latest_video)

자, 이제 코드를 하나씩 살펴볼까요?
"""

from airflow import DAG
import pendulum
from datetime import datetime, timedelta

# ==============================================================================
# 1. 도구 상자 (Imports)
# ==============================================================================
# 로봇들이 일을 하려면 도구가 필요해요.
# PythonOperator는 "파이썬 코드를 실행할 수 있는 로봇 팔"이에요.
from airflow.providers.standard.operators.python import PythonOperator


# ==============================================================================
# 2. 기본 규칙 (Default Args)
# ==============================================================================
# 모든 로봇들이 지켜야 할 공통 약속이에요.
default_args = {
    "owner": "geon_yul",       # 이 지도의 주인님 이름이에요.
    "retries": 2,              # 로봇이 실수로 넘어지면, 2번까지는 다시 일어나서 시도해보라고 해요.
    "retry_delay": timedelta(seconds=300), # 다쳤으면 300초(5분) 정도 쉬었다가 다시 하래요.
}

# ==============================================================================
# 3. 보물 지도 그리기 (Pipeline Definition)
# ==============================================================================
# with DAG(...) as dag: << 여기가 바로 지도의 시작이에요!
with DAG(
    dag_id="pipeline__eng_youtube_transfer", # 지도의 이름이에요. 에어플로우 화면에 이 이름으로 떠요.
    default_args=default_args,               # 위에서 정한 약속을 지키기로 해요.
    schedule="@hourly",                      # "한 시간마다 한 번씩" 이 지도를 펼쳐서 일을 시작해요.
    catchup=False,                           # 우리가 잠자고 있을 때 놓친 일들은 굳이 다시 하지 않기로 해요. (새로운 것만!)
    tags=[],                                 # 태그(꼬리표)는 달지 않았어요.
) as dag:

    # ==========================================================================
    # 4. 로봇 친구들 소환 (Task Definitions)
    # ==========================================================================

    # 첫 번째 로봇: 영상 찾아오기 대장
    fetch_videos = PythonOperator(
        task_id="fetch_videos",  # 이 로봇의 이름표예요.
        
        # 이 로봇이 해야 할 동작(함수)이에요.
        # "eng_youtube.video_ops.fetch_latest_videos" 라는 책에 적힌 대로 행동해요.
        # 실제로는 여기서 파이썬 코드가 실행돼요!
        python_callable=lambda: print(
            "Executing eng_youtube.video_ops.fetch_latest_videos"
        ),
        
        # 로봇에게 줄 준비물이에요. 
        # "이 채널(channel_id)에서, 영상 3개(limit)만 찾아와!" 라고 주문서를 줬어요.
        op_kwargs={"channel_id": "UCtoNXlIegvxkvf5Ji8S57Ag", "limit": 3},
    )

    # 두 번째 로봇: 대본(스크립트) 추출하기 대장
    process_latest_video = PythonOperator(
        task_id="process_latest_video", # 이 로봇의 이름표예요.
        
        # "eng_youtube.video_ops.process_video_pipeline" 책에 적힌 대로 영상을 처리해요.
        # 영상의 **전체 대본(Full Script)**을 다운로드 받아서 파일로 저장해줘요!
        python_callable=lambda: print(
            "Executing eng_youtube.video_ops.process_video_pipeline"
        ),
        
        # "이 비디오(video_12345)의 대본을 뽑아줘!" 라고 알려줬어요.
        # (나중에는 첫 번째 로봇이 찾아온 비디오를 자동으로 넘겨받게 될 거예요!)
        op_kwargs={"video_id": "video_12345"},
    )

    # ==========================================================================
    # 5. 순서 정하기 (Dependencies)
    # ==========================================================================
    # 여기가 제일 중요해요! 로봇들이 동시에 우루루 나가면 엉망이 되겠죠?
    # ">>" 화살표가 순서를 알려줘요.
    
    # "영상 찾아오기(fetch_videos)"가 먼저 끝나야 -> "대본 추출하기(process_latest_video)"를 시작해요.
    # 만약 영상을 못 찾아오면? 대본 뽑는 로봇은 시작도 안 하고 기다릴 거예요.
    
    fetch_videos >> process_latest_video
